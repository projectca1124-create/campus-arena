...
import { getSocket, disconnectSocket, emitTypingStart, emitTypingStop, joinGroupRoom, leaveGroupRoom } from '@/lib/socket-client'
...
function getGroupAbbr(group: Group): string {
  const name = group.name.trim(); const nameLower = name.toLowerCase()
  if (group.identifier && group.identifier.includes('-arena')) {
    const code = group.identifier.split('-arena')[0]; return code.toUpperCase()
  }
  ...
  if (group.identifier && group.identifier.startsWith('major-')) {
    const abbrs: Record<string, string> = {
      'computer science':'CS','data science':'DS','electrical engineering':'EE','mechanical engineering':'ME',
      'civil engineering':'CE','chemical engineering':'ChE','business administration':'BA','information technology':'IT',
      'information systems':'IS','software engineering':'SE','computer engineering':'CpE','biomedical engineering':'BME',
      'industrial engineering':'IE','aerospace engineering':'AE','mathematics':'MATH','math':'MATH','physics':'PHY',
      'chemistry':'CHEM','biology':'BIO','psychology':'PSY','nursing':'NUR','finance':'FIN','accounting':'ACCT',
      'marketing':'MKT','economics':'ECON','english':'ENG','history':'HIST','political science':'POLS',
      'communications':'COMM','communication':'COMM','sociology':'SOC','anthropology':'ANTH','philosophy':'PHIL',
      'architecture':'ARCH','art':'ART','music':'MUS','theatre':'THE','theater':'THE','journalism':'JOUR',
      'public health':'PH','criminal justice':'CJ','social work':'SW','education':'EDU','kinesiology':'KIN',
      'nutrition':'NUT','environmental science':'ENV','geoscience':'GEO','geology':'GEO','linguistics':'LING',
      'international studies':'IS','urban planning':'UP','supply chain':'SCM','management':'MGMT',
      'human resources':'HR','neuroscience':'NEUR','biochemistry':'BIOC','statistics':'STAT',
    }
    const match = abbrs[nameLower]; if (match) return match
...
export default function HomePage() {
  ...
  // ─── Local storage ────────────────────────────────────────────────
  useEffect(() => {
    try { const s = localStorage.getItem('groupLastRead'); if (s) setGroupLastRead(JSON.parse(s)) } catch {}
    try { const s = localStorage.getItem('mutedChats'); if (s) setMutedChats(new Set(JSON.parse(s))) } catch {}
  }, [])

  const markGroupRead = (groupId: string) => {
    setGroupLastRead(prev => {
      const next = { ...prev, [groupId]: new Date().toISOString() }
      localStorage.setItem('groupLastRead', JSON.stringify(next)); return next
    })
  }
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (!(e.target as HTMLElement).closest('[data-sidebar-menu]')) setSidebarMenuId(null)
    }
  ...
  // ─── Group helpers ────────────────────────────────────────────────
  const getUserRoleInGroup = (group: Group | null) => {
    if (!group || !user) return 'member'
    return group.members?.find(m => m.userId === user.id || m.user?.id === user.id)?.role || 'member'
  }
  const getInviteLink = (group: Group) => typeof window === 'undefined' ? '' : `${window.location.origin}/home?joinCode=${group.inviteCode || ''}`
  const copyInviteLink = (group: Group) => {
    navigator.clipboard.writeText(getInviteLink(group)).then(() => { setCopiedInviteLink(true); setTimeout(() => setCopiedInviteLink(false), 2000) })
  ...
  const handlePinGroup = async (groupId: string) => {
    ...
    const prev = pinnedGroupId; setPinnedGroupId(pinnedGroupId === groupId ? null : groupId)
    try {
  ...
  const handleShareGroupViaDM = (conv: DMConversation, group: Group) => {
    ...
    fetch('/api/dm', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: `Hey! Join my group "${group.name}" on Campus Arena: ${getInviteLink(group)}`, receiverId: conv.user.id }) })
    setShareGroupViaDM(null)
  }
  const handleDeleteGroup = async () => {
    if (!selectedChat || !user) return; setIsDeletingGroup(true)
    try {
      const res = await fetch(`/api/groups/${selectedChat.id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id }) })
      if (res.ok) { setGroups(prev => prev.filter(g => g.id !== selectedChat.id)); setSelectedChat(null); setShowDeleteGroupModal(false); setShowHeaderMenu(false) }
    } catch (err) { console.error(err) } finally { setIsDeletingGroup(false) }
  }
  const handleLeaveGroup = async () => {
    if (!selectedChat || !user) return
    try {
      const res = await fetch('/api/groups/leave', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, groupId: selectedChat.id }) })
      if (res.ok) { setGroups(prev => prev.filter(g => g.id !== selectedChat.id)); setSelectedChat(null); setShowLeaveGroupModal(false); setShowHeaderMenu(false) }
    } catch (err) { console.error(err) }
  }
  const handleGroupAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]; if (!file || !selectedChat || !user) return
    e.target.value = ''; if (file.size > 5 * 1024 * 1024) { alert('Image must be under 5MB'); return }
    setIsUploadingGroupAvatar(true)
    try {
      const compressed = await new Promise<string>((resolve, reject) => {
        const reader = new FileReader(); reader.onload = (ev) => {
          const img = new Image(); img.onload = () => {
            const canvas = document.createElement('canvas'); const scale = Math.min(1, 300 / img.width)
      ...
      const res = await fetch(`/api/groups/${selectedChat.id}/avatar`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, icon: compressed }) })
      if (res.ok) {
        const data = await res.json(); const updatedIcon = data.group?.icon || compressed
        setGroups(prev => prev.map(g => g.id === selectedChat.id ? { ...g, icon: updatedIcon } : g))
        setSelectedChat(prev => prev ? { ...prev, icon: updatedIcon } : prev)
      }
    } catch (err) { console.error(err) } finally { setIsUploadingGroupAvatar(false) }
  }
  const handleLoadJoinRequests = async (groupId: string) => {
    if (!user) return
    try {
      const res = await fetch(`/api/groups/${groupId}?adminId=${user.id}`)
      if (res.ok) { const data = await res.json(); setJoinRequests(data.requests || []); setShowJoinRequests(true) }
    } catch {}
  }
  const handleApproveReject = async (requesterId: string, action: 'approve' | 'reject') => {
    if (!selectedChat || !user) return
    try {
      const res = await fetch(`/api/groups/${selectedChat.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminId: user.id, requesterId, action }) })
      if (res.ok) {
        const data = await res.json(); setJoinRequests(prev => prev.filter(r => r.user.id !== requesterId))
        if (action === 'approve' && data.group) { setGroups(prev => prev.map(g => g.id === selectedChat.id ? data.group : g)); setSelectedChat(data.group) }
      }
    } catch {}
  }
  const handleJoinGroup = async () => {
    ...
    try {
      ...
      if (d.alreadyMember) { setJoinError("You're already a member!"); return }
      if (d.pending) { setShowJoinModal(false); setJoinCode(''); setPendingApproval({ groupName: d.groupName }); return }
      setGroups(p => [...p, d.group]); setShowJoinModal(false); setJoinCode('')
      // Join the socket room for this new group
      joinGroupRoom(d.group.id)
      handleSelectChat(d.group)
    } catch { setJoinError('Something went wrong.') } finally { setIsJoining(false) }
  }
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setUser(currentUser); userRef.current = currentUser
        setPinnedGroupId(currentUser.pinnedGroupId || null)

        // Connect socket immediately on load
        ...
        try {
          ...
          const groupIdParam = params.get('groupId'); const approveUserParam = params.get('approveUser')
          if (groupIdParam && !approveUserParam) {
            window.history.replaceState({}, '', '/home')
            setTimeout(() => {
              setGroups(prev => { const g = prev.find(g => g.id === groupIdParam); if (g) { setSelectedChat(g); setSelectedDM(null); loadMessages(g.id) }; return prev })
            }, 300)
          }
          if (groupIdParam && approveUserParam) {
            window.history.replaceState({}, '', '/home')
            setTimeout(async () => {
              const s = localStorage.getItem('user'); if (!s) return; const u2 = JSON.parse(s)
              const res = await fetch(`/api/groups/${groupIdParam}?adminId=${u2.id}`)
              if (res.ok) {
                const data = await res.json(); setJoinRequests(data.requests || []); setShowJoinRequests(true)
                const gr2 = await fetch(`/api/groups?userId=${u2.id}`)
                if (gr2.ok) { const gd = await gr2.json(); const g = (gd.groups || []).find((g: any) => g.id === groupIdParam); if (g) { setSelectedChat(g); loadMessages(g.id) } }
              }
            }, 500)
          }
          const tabParam = params.get('tab'); if (tabParam === 'dms') setActiveTab('dms')
          const openDMId = params.get('openDM')
          if (openDMId) {
            const dmDataStr = sessionStorage.getItem('openDM'); let dmData: any = null
            if (dmDataStr) { dmData = JSON.parse(dmDataStr); sessionStorage.removeItem('openDM') }
            else { const dmName = params.get('dmName') || ''; const np = dmName.split(' '); dmData = { id: openDMId, firstName: np[0] || '', lastName: np.slice(1).join(' ') || '' } }
            setActiveTab('dms')
            ...
            if (existing) {
              ...
              setDmConversations(prev => prev.map(c => c.user.id === existing.user.id ? { ...c, unreadCount: 0 } : c))
            } else {
  ...
  // Single persistent effect. Handles ALL real-time events.
  useEffect(() => {
    if (!user) return
    ...
    socket.on('user:online', (data: { userId: string }) => {
      setOnlineUsers(prev => { const next = new Set(prev); next.add(data.userId); return next })
    })
    ...
    // ── Group messages ─────────────────────────────────────────────
    socket.on('group:message:new', (data: { message: Message }) => {
      const msg = data.message
      if (selectedChatRef.current?.id === msg.groupId) {
        setMessages(prev => {
          if (prev.some(m => m.id === msg.id)) return prev
          const tempIdx = prev.findIndex(m => m.id.startsWith('temp_') && m.userId === msg.userId && m.content === msg.content)
          if (tempIdx >= 0) { const next = [...prev]; next[tempIdx] = msg; return next }
          if (msg.userId !== user.id) playReceiveSound()
          return [...prev, msg]
        })
      }
      // Update group sidebar last message
      setGroups(prev => prev.map(g => {
        if (g.id !== msg.groupId) return g
        return { ...g, messages: [...(g.messages || []).slice(-1), msg] }
      }))
      // Play sound if not in this group and not muted
      if (selectedChatRef.current?.id !== msg.groupId && msg.userId !== user.id) {
        if (!mutedChatsRef.current.has(`group_${msg.groupId}`)) playReceiveSound()
      }
    })
    socket.on('group:message:edited', (data: { messageId: string; content: string }) => {
    ...
    socket.on('group:message:reaction', (data: { messageId: string; reactions: Reaction[] }) => {
      setMessages(prev => prev.map(m => m.id === data.messageId ? { ...m, reactions: data.reactions } : m))
    })

    // ── DM messages ────────────────────────────────────────────────
    socket.on('dm:new', (data: { message: DMMessage; from: any }) => {
      const msg = data.message
      const isOpen = selectedDMRef.current?.user.id === msg.senderId

      if (isOpen) {
        setDmMessages(prev => {
          if (prev.some(m => m.id === msg.id)) return prev
          const tempIdx = prev.findIndex(m => m.id.startsWith('temp_') && m.senderId === msg.senderId && m.content === msg.content)
          if (tempIdx >= 0) { const next = [...prev]; next[tempIdx] = msg; return next }
          playReceiveSound()
          return [...prev, msg]
        })
        setDmConversations(prev => prev.map(c =>
          c.user.id === msg.senderId
            ? { ...c, lastMessage: msg.content || '📎 Attachment', lastMessageAt: msg.createdAt, unreadCount: 0 }
            : c
        ))
        // Mark as read immediately since chat is open
        socket.emit('dm:read', { senderId: msg.senderId })
      } else {
        if (!mutedChatsRef.current.has(`dm_${msg.senderId}`)) playReceiveSound()
        setDmConversations(prev => {
          const idx = prev.findIndex(c => c.user.id === msg.senderId)
          if (idx >= 0) {
            const next = [...prev]
            next[idx] = { ...next[idx], lastMessage: msg.content || '📎 Attachment', lastMessageAt: msg.createdAt, unreadCount: (next[idx].unreadCount || 0) + 1 }
            return next.sort((a, b) => new Date(b.lastMessageAt).getTime() - new Date(a.lastMessageAt).getTime())
          }
          return [{ user: data.from, lastMessage: msg.content || '📎 Attachment', lastMessageAt: msg.createdAt, unreadCount: 1 }, ...prev]
        })
      }
    })

    // Sent confirmation (handles multi-device / multi-tab)
    socket.on('dm:sent', (data: { message: DMMessage }) => {
      const msg = data.message
      setDmMessages(prev => {
        if (prev.some(m => m.id === msg.id)) return prev
        const tempIdx = prev.findIndex(m => m.id.startsWith('temp_') && m.senderId === msg.senderId && m.content === msg.content)
    ...
    socket.on('dm:edited', (data: { messageId: string; content: string }) => {
      setDmMessages(prev => prev.map(m => m.id === data.messageId ? { ...m, content: data.content } : m))
    })
    socket.on('dm:deleted', (data: { messageId: string }) => {
      setDmMessages(prev => prev.filter(m => m.id !== data.messageId))
    })
    socket.on('dm:read:receipt', (data: { readBy: string; readAt: string }) => {
      if (data.readBy !== user.id) {
        setDmReadStatus(true)
        setDmMessages(prev => prev.map(m => m.senderId === user.id ? { ...m, read: true } : m))
      }
    })
    socket.on('dm:reaction', (data: { messageId: string; reactions: Reaction[] }) => {
      setDmMessages(prev => prev.map(m => m.id === data.messageId ? { ...m, reactions: data.reactions } : m))
    })

    // ── Typing ─────────────────────────────────────────────────────
    socket.on('typing:update', (data: { userId: string; userName: string; isTyping: boolean; fromUserId?: string }) => {
      const typingUserId = data.fromUserId || data.userId
      if (typingUserId === user.id) return
      setTypingUsers(prev => {
        const next = { ...prev }
        if (data.isTyping) {
          if (next[typingUserId]?.timeout) clearTimeout(next[typingUserId].timeout)
          const timeout = setTimeout(() => {
            setTypingUsers(p => { const n = { ...p }; delete n[typingUserId]; return n })
          }, 3000)
          next[typingUserId] = { name: data.userName, timeout }
        } else {
          if (next[typingUserId]?.timeout) clearTimeout(next[typingUserId].timeout)
    ...
    socket.on('group:deleted', (data: { groupId: string }) => {
      ...
      if (selectedChatRef.current?.id === data.groupId) setSelectedChat(null)
    })
    socket.on('group:member:left', (data: { userId: string; groupId: string }) => {
      setGroups(prev => prev.map(g => g.id !== data.groupId ? g : { ...g, members: g.members.filter(m => m.userId !== data.userId) }))
      setSelectedChat(prev => prev?.id !== data.groupId ? prev : { ...prev, members: prev.members.filter(m => m.userId !== data.userId) })
    })
    socket.on('group:approved', (data: { groupId: string }) => {
      const s = localStorage.getItem('user'); if (!s) return
      const u = JSON.parse(s)
      fetch(`/api/groups?userId=${u.id}`).then(r => r.json()).then(d => {
        const g = (d.groups || []).find((g: Group) => g.id === data.groupId)
        if (g) { setGroups(prev => prev.some(x => x.id === g.id) ? prev : [...prev, g]); joinGroupRoom(g.id) }
      }).catch(() => {})
    })

    // ── Reconnect — re-fetch missed messages ───────────────────────
    socket.on('connect', () => {
      // Re-fetch messages missed during disconnect
      if (selectedChatRef.current && messagesRef.current.length > 0) {
        const last = messagesRef.current[messagesRef.current.length - 1]
        if (!last.id.startsWith('temp_')) {
          fetch(`/api/messages?groupId=${selectedChatRef.current.id}&after=${encodeURIComponent(last.createdAt)}`)
            .then(r => r.json()).then(d => {
              if (d.messages?.length) {
                setMessages(prev => { const ids = new Set(prev.map(m => m.id)); const fresh = d.messages.filter((m: any) => !ids.has(m.id)); return fresh.length ? [...prev, ...fresh] : prev })
              }
            }).catch(() => {})
        }
      }
      if (selectedDMRef.current && dmMessagesRef.current.length > 0) {
        ...
        if (!last.id.startsWith('temp_')) {
          fetch(`/api/dm?otherUserId=${selectedDMRef.current.user.id}&userId=${user.id}&after=${encodeURIComponent(last.createdAt)}`)
            .then(r => r.json()).then(d => {
              if (d.messages?.length) {
                setDmMessages(prev => { const ids = new Set(prev.map(m => m.id)); const fresh = d.messages.filter((m: any) => !ids.has(m.id)); return fresh.length ? [...prev, ...fresh] : prev })
              }
            }).catch(() => {})
        }
      }
    ...
    return () => {
      socket.off('presence:init'); socket.off('user:online'); socket.off('user:offline')
      socket.off('group:message:new'); socket.off('group:message:edited')
      socket.off('group:message:deleted'); socket.off('group:message:reaction')
      socket.off('dm:new'); socket.off('dm:sent'); socket.off('dm:edited')
      socket.off('dm:deleted'); socket.off('dm:read:receipt'); socket.off('dm:reaction')
      socket.off('typing:update'); socket.off('group:deleted')
      socket.off('group:member:left'); socket.off('group:approved'); socket.off('connect')
    }
  }, [user?.id])

  // ── Typing state cleanup when chat changes ─────────────────────
  useEffect(() => { setTypingUsers({}); setDmReadStatus(false) }, [selectedChat?.id, selectedDM?.user.id])

  // ── Polling fallback (15s — only if socket disconnected) ───────
  useEffect(() => {
    const poll = async () => {
      ...
      try {
        if (selectedChatRef.current && messagesRef.current.length > 0) {
          ...
          const res = await fetch(`/api/messages?groupId=${selectedChatRef.current.id}&after=${encodeURIComponent(last.createdAt)}`)
          if (res.ok) { const d = await res.json(); if (d.messages?.length) { setMessages(prev => { const ids = new Set(prev.map(m => m.id)); const n = d.messages.filter((m: any) => !ids.has(m.id)); return n.length ? [...prev, ...n] : prev }) } }
        }
        if (selectedDMRef.current && dmMessagesRef.current.length > 0) {
          const last = dmMessagesRef.current[dmMessagesRef.current.length - 1]
          if (last.id.startsWith('temp_')) return
          const res = await fetch(`/api/dm?otherUserId=${selectedDMRef.current.user.id}&after=${encodeURIComponent(last.createdAt)}`)
          if (res.ok) { const d = await res.json(); if (d.messages?.length) { setDmMessages(prev => { const ids = new Set(prev.map(m => m.id)); const n = d.messages.filter((m: any) => !ids.has(m.id)); return n.length ? [...prev, ...n] : prev }) } }
        }
      } catch {}
    }
    ...
    const handler = (e: Event) => {
      ...
      if (type === 'dm' && targetUserId) {
        ...
        if (existing) { handleSelectDM(existing) } else {
          const np = (dmName || '').split(' ')
          setSelectedDM({ user: { id: targetUserId, firstName: np[0] || '', lastName: np.slice(1).join(' ') || '' }, lastMessage: '', lastMessageAt: new Date().toISOString(), unreadCount: 0 })
          setSelectedChat(null); setDmMessages([]); loadDMMessages(targetUserId)
        }
        if (isMobile) setMobileView('chat')
      }
      if (type === 'group' && groupId) {
        setActiveTab('groups'); const group = groups.find(g => g.id === groupId)
        if (group) { handleSelectChat(group); if (isMobile) setMobileView('chat') }
      }
      if (type === 'tab' && tab) setActiveTab(tab)
    }
  ...
  const sendTypingEvent = useCallback(() => {
    if (!user) return
    const now = Date.now(); if (now - lastTypingSentRef.current < 2000) return
    lastTypingSentRef.current = now
    if (selectedChat) emitTypingStart('group', selectedChat.id)
    else if (selectedDM) emitTypingStart('dm', selectedDM.user.id)
    if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current)
    typingTimeoutRef.current = setTimeout(() => {
      if (selectedChatRef.current) emitTypingStop('group', selectedChatRef.current.id)
      else if (selectedDMRef.current) emitTypingStop('dm', selectedDMRef.current.user.id)
    }, 3000)
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // Send via Socket.io directly — server saves to DB and broadcasts
    const socket = getSocket()
    socket.emit('group:message:send', {
      groupId: selectedChat.id, content: savedMsg,
      imageUrl: savedImg || undefined, fileUrl: savedFile?.url, fileName: savedFile?.name, fileType: savedFile?.type,
  ...
  const handleSendDM = async (e: React.FormEvent) => {
    ...
    const socket = getSocket()
    socket.emit('dm:send', {
      receiverId: selectedDM.user.id, content: savedMsg,
      imageUrl: savedImg || undefined, fileUrl: savedFile?.url, fileName: savedFile?.name, fileType: savedFile?.type,
      replyToId: savedReply?.id,
    }, (res: any) => {
      if (res?.message) {
        setDmMessages(prev => prev.map(m => m.id === tempId ? res.message : m))
        setDmConversations(prev => {
          const exists = prev.some(c => c.user.id === selectedDM!.user.id)
          const updated = { user: selectedDM!.user, lastMessage: res.message.content || '📎 Attachment', lastMessageAt: res.message.createdAt, unreadCount: 0 }
          if (exists) return prev.map(c => c.user.id === selectedDM!.user.id ? { ...c, ...updated } : c).sort((a, b) => new Date(b.lastMessageAt).getTime() - new Date(a.lastMessageAt).getTime())
          return [updated, ...prev]
        })
      } else {
        // Fallback to REST
        ...
        fetch('/api/dm', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
          .then(r => r.ok ? r.json() : null).then(data => {
            if (data?.message) setDmMessages(prev => prev.map(m => m.id === tempId ? data.message : m))
  ...
  const handleGifSelect = (url: string, type: 'gif' | 'sticker') => {
    ...
    } else { setImagePreview(url); setTimeout(() => { const form = document.getElementById('chat-form') as HTMLFormElement; form?.requestSubmit() }, 50) }
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const d = await res.json()
        setGroups(p => [...p, d.group])
        // Join the socket room for this new group
        joinGroupRoom(d.group.id)
        setShowCreateGroup(false); setGroupForm({ name: '', description: '', visibility: 'public', icon: '' }); setGroupErrors({})
      }
  ...
  const handleSelectChat = (group: Group) => {
    ...
    const socket = getSocket()
    socket.emit('dm:read', { senderId: group.id }) // no-op for groups but consistent
  }
  const handleSelectDM = (conv: DMConversation) => {
    ...
    const socket = getSocket()
    socket.emit('dm:read', { senderId: conv.user.id })
  }
  ...
  const handleDeleteForEveryone = async (msg: Message | DMMessage) => {
    ...
    const socket = getSocket()
    socket.emit('group:message:delete', { messageId: msg.id, groupId: selectedChat.id }, (res: any) => {
  ...
  const handleDeleteMessage = async (msg: Message | DMMessage) => {
    ...
    const socket = getSocket()
    socket.emit('dm:delete', { messageId: msg.id, receiverId: selectedDM.user.id }, (res: any) => {
      if (res?.success) setDmMessages(prev => prev.filter(m => m.id !== msg.id))
    })
  }
  ...
  const handleSaveEdit = async () => {
    ...
    const socket = getSocket()
    if (isGroupMsg && selectedChat) {
      socket.emit('group:message:edit', { messageId: editingMessage.id, content: editingMessage.content, groupId: selectedChat.id }, (res: any) => {
        if (res?.success) setMessages(prev => prev.map(m => m.id === editingMessage.id ? { ...m, content: editingMessage.content } : m))
      })
    } else if (selectedDM) {
      socket.emit('dm:edit', { messageId: editingMessage.id, content: editingMessage.content, receiverId: selectedDM.user.id }, (res: any) => {
        if (res?.success) setDmMessages(prev => prev.map(m => m.id === editingMessage.id ? { ...m, content: editingMessage.content } : m))
      })
    }
  ...
  const forwardToDM = (conv: DMConversation) => {
    if (!forwardingMessage || !user) return
    const body: any = { content: forwardingMessage.content || '', receiverId: conv.user.id }
    if (forwardingMessage.imageUrl) body.imageUrl = forwardingMessage.imageUrl
    fetch('/api/dm', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    setForwardingMessage(null)
  }
  ...
  const insertEmoji = (emoji: string) => {
    if (selectedChat) setNewMessage(p => p + emoji); else if (selectedDM) setNewDMMessage(p => p + emoji)
  ...
  const getAvailableYears = () => !selectedChat ? [] : [...new Set(selectedChat.members?.map(m => m.user.year).filter(Boolean) as string[])]
  const getAvailableSemesters = () => !selectedChat ? [] : [...new Set(selectedChat.members?.map(m => m.user.semester).filter(Boolean) as string[])]
  const toggleGroupInfo = () => { setShowGroupInfo(p => !p); setMemberSearch(''); setMemberYearFilter(''); setMemberSemesterFilter('') }
  const shouldShowDateSeparator = (msgs: { createdAt: string }[], idx: number) => idx === 0 || new Date(msgs[idx - 1].createdAt).toDateString() !== new Date(msgs[idx].createdAt).toDateString()
  ...
  if (isLoading) return (
    <div className="flex items-center justify-center h-screen">
      <div className="text-center"><div className="w-12 h-12 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin mx-auto mb-4" /><p className="text-gray-500 text-sm font-medium">Loading Campus Arena...</p></div>
    </div>
  )
  ...
  const renderAttachment = (msg: { imageUrl?: string; fileUrl?: string; fileName?: string; fileType?: string }, isOwn: boolean) => (
    <>
      ...
      {msg.fileUrl && msg.fileName && (
        <a href={msg.fileUrl} download={msg.fileName} className={`flex items-center gap-3 mt-1.5 mb-1 px-3 py-2.5 rounded-xl border transition-all ${isOwn ? 'border-indigo-500/30 bg-indigo-50 hover:bg-indigo-600/25' : 'border-gray-700 bg-gray-50 hover:bg-gray-100'}`}>
  ...
  const renderReplyPreview = (replyTo: ReplyTo | null | undefined, isOwn: boolean) => {
    ...
    return (
      <div className="flex items-stretch mb-2 rounded-[10px] overflow-hidden" style={{ background: isOwn ? 'rgba(255,255,255,0.12)' : 'rgba(99,102,241,0.06)', borderLeft: `3px solid ${isOwn ? 'rgba(255,255,255,0.75)' : '#6366f1'}` }}>
        <div className="px-2.5 py-1.5 flex-1 min-w-0 overflow-hidden">
          <p className="text-[11px] font-bold leading-tight mb-0.5 truncate" style={{ color: isOwn ? 'rgba(255,255,255,0.95)' : '#6366f1' }}>{replyName}</p>
          {replyTo.imageUrl && !replyTo.content ? <p className="text-[11px]" style={{ color: isOwn ? 'rgba(255,255,255,0.55)' : '#9ca3af' }}>📷 Photo</p>
            : replyTo.content ? <p className="text-[11px] leading-snug line-clamp-2" style={{ color: isOwn ? 'rgba(255,255,255,0.6)' : '#6b7280' }}>{replyTo.content}</p> : null}
        </div>
        {replyTo.imageUrl && <img src={replyTo.imageUrl} alt="" className="flex-shrink-0 object-cover" style={{ width: 46, height: 46 }} />}
      </div>
    )
  }
  ...
  const renderMessageContent = (content: string, isOwn: boolean) => {
    ...
    return parts.map((part, i) => {
      if (!part.match(/^https?:\/\//)) return <React.Fragment key={i}>{part}</React.Fragment>
      const joinCodeMatch = part.match(/joinCode=([a-z0-9]+)/i)
      if (joinCodeMatch) {
        return (
          <div key={i} className={`my-1 rounded-xl overflow-hidden border ${isOwn ? 'border-white/20' : 'border-indigo-100'}`} style={isOwn ? { background: 'rgba(255,255,255,0.12)' } : { background: 'linear-gradient(135deg,#eef2ff,#f5f3ff)' }}>
            <div className="px-3 py-2.5 flex items-center gap-2.5">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${isOwn ? 'bg-white/20' : 'bg-indigo-100'}`}><span className="text-base">👥</span></div>
              ...
              <button onClick={e => { e.stopPropagation(); setJoinCode(joinCodeMatch[1]); setShowJoinModal(true) }} className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-[12px] font-bold transition-all ${isOwn ? 'bg-white text-indigo-600 hover:bg-indigo-50' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}>Join</button>
            </div>
          </div>
        )
      }
      return <a key={i} href={part} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className={`underline break-all ${isOwn ? 'text-indigo-100 hover:text-white' : 'text-indigo-600 hover:text-indigo-800'}`}>{part}</a>
    })
  }

  const renderMsgBubble = (msg: any, idx: number, allMsgs: any[], isDM: boolean) => {
    const isSystemMsg = msg.isSystemMessage || (typeof msg.content === 'string' && !msg.imageUrl && !msg.fileUrl && (msg.content.endsWith(' joined the group') || msg.content.endsWith(' left the group') || msg.content.endsWith(' was added to the group') || msg.content.endsWith(' was removed from the group')))
      ...
      <React.Fragment key={msg.id}>
        ...
        <div className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'} ${isTemp ? 'opacity-60' : ''} ${isCons ? 'mt-0.5' : 'mt-3'}`}>
          ...
          <div className={`flex gap-2 max-w-[65%] ${isOwn ? 'flex-row-reverse' : ''} group/msg`}>
            ...
            {!isOwn && isCons && <div className="w-8 flex-shrink-0" />}
            <div className="relative">
              <div className={`px-4 py-2.5 text-sm leading-relaxed ${isOwn ? 'text-white' : 'text-gray-900'}`}
                style={isOwn ? { background: 'linear-gradient(135deg,#6366f1,#7c3aed)', borderRadius: '20px 20px 6px 20px', boxShadow: '0 2px 12px rgba(99,102,241,0.25)' } : { background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.8)', borderRadius: '20px 20px 20px 6px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                {renderReplyPreview(msg.replyTo, isOwn)}
                {editingMessage?.id === msg.id ? (
                  <div className="flex flex-col gap-1.5" style={{ minWidth: 200, maxWidth: 340 }}>
                    <textarea value={editingMessage?.content || ''} onChange={e => { setEditingMessage(prev => prev ? { ...prev, content: e.target.value } : prev); e.target.style.height = 'auto'; e.target.style.height = Math.min(e.target.scrollHeight, 120) + 'px' }}
                      onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSaveEdit() }; if (e.key === 'Escape') setEditingMessage(null) }}
                      autoFocus rows={2} className={`w-full px-2 py-1 text-sm rounded-lg outline-none resize-none ${isOwn ? 'bg-white/20 text-white placeholder-white/50' : 'bg-gray-50 text-gray-900'}`} style={{ minHeight: 36, maxHeight: 120 }} />
          ...
          <p className={`text-[10px] text-gray-600 mt-0.5 flex items-center gap-1 ${isOwn ? 'mr-1 justify-end' : 'ml-12'}`}>
  ...
  const handleSelectChatMobile = (chat: any) => { handleSelectChat(chat); if (isMobile) setMobileView('chat') }
  const handleSelectDMMobile = (conv: DMConversation) => { handleSelectDM(conv); if (isMobile) setMobileView('chat') }

  return (
    <div className="flex h-screen bg-gray-50" style={{ position: 'relative', overflow: 'hidden' }}
      ...
      {isMobile && mobileView !== 'chat' && (
        <div className="absolute top-0 left-0 right-0 h-14 flex items-center justify-between px-4 z-30 border-b" style={{ background: 'white', borderColor: '#e5e7eb' }}>
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-lg flex items-center justify-center"><span className="text-white font-bold text-[10px]">CA</span></div>
      ...
      {!isMobile && (
        <aside className="w-[220px] flex flex-col flex-shrink-0 border-r" style={{ background: 'white', borderColor: '#e5e7eb' }}>
          <div className="px-5 py-5">
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                ...
                {user?.university && <span className="text-[11px] text-indigo-500 font-semibold truncate block leading-tight mt-0.5">{user.university.includes('.') ? user.university.split('.')[0].toUpperCase() : user.university}</span>}
              </div>
            </div>
          </div>
          <nav className="flex-1 overflow-y-auto px-3 pt-2 space-y-1">
            ...
            <NavItem icon={<Gamepad2 className="w-[18px] h-[18px]" />} label="Campus Games" onClick={() => router.push('/home/campus-games')} />
          </nav>
          <div className="px-3 py-4" style={{ borderTop: '1px solid #e5e7eb' }}>
            <button onClick={() => setShowLogoutModal(true)} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-gray-500 hover:text-red-600 hover:bg-red-50 transition-all text-sm font-medium"><LogOut className="w-[18px] h-[18px]" /><span>Log out</span></button>
          </div>
        </aside>
      )}
      ...
      <aside className="flex flex-col flex-shrink-0 border-r"
        style={{ background: 'white', borderColor: '#e5e7eb', width: isMobile ? '100%' : '340px', display: 'flex', position: isMobile ? 'absolute' : 'relative', top: isMobile ? '56px' : 0, left: 0, bottom: isMobile ? '60px' : 0, zIndex: isMobile ? 20 : 'auto', overflowY: isMobile ? 'auto' : undefined, transform: isMobile && mobileView !== 'list' ? 'translateX(-100%)' : 'translateX(0)', transition: isMobile ? 'transform 0.3s cubic-bezier(0.4,0,0.2,1)' : undefined, visibility: isMobile && mobileView !== 'list' ? 'hidden' : 'visible' }}>
        <div className="px-5 pb-4" style={{ borderBottom: '1px solid #e5e7eb', paddingTop: '20px' }}>
          ...
          <div className="relative flex rounded-xl p-1" style={{ background: '#f3f4f6' }}>
            <div className="absolute top-1 bottom-1 rounded-lg shadow-lg transition-all duration-300 ease-out" style={{ background: '#4f46e5', width: 'calc(50% - 4px)', left: activeTab === 'groups' ? '4px' : 'calc(50%)' }} />
            <button onClick={() => setActiveTab('groups')} className={`relative z-10 flex-1 flex items-center justify-center gap-1.5 py-2.5 text-sm font-bold rounded-lg transition-colors duration-300 ${activeTab === 'groups' ? 'text-white' : 'text-gray-500 hover:text-gray-600'}`}>
              Groups{groups.reduce((s, g) => s + getGroupUnread(g), 0) > 0 ? ` (${groups.reduce((s, g) => s + getGroupUnread(g), 0)})` : ''}
            </button>
          ...
          {activeTab === 'groups' && (
            <div>
              <div className="flex items-center justify-between mb-3 px-1">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setShowJoinModal(true)} className="text-[12px] text-indigo-500 font-bold hover:text-indigo-600 flex items-center gap-0.5"><UserPlus className="w-3.5 h-3.5" />Join</button>
              ...
              <div className="space-y-1.5">
                {filteredGroups.map(group => {
                  const isSel = selectedChat?.id === group.id; const chatId = `group_${group.id}`
                  const muted = isMuted(chatId); const isPinned = pinnedGroupId === group.id
                  const isPrivate = group.visibility === 'private'; const isDef = group.isDefault
                  return (
                    <div key={group.id} className="relative group/item" data-sidebar-menu>
                      <button onClick={() => handleSelectChatMobile(group)} className={`w-full text-left p-3 rounded-xl transition-all flex items-center gap-3 ${isSel ? 'shadow-lg shadow-indigo-600/10' : 'hover:bg-gray-50'}`} style={isSel ? { background: '#4f46e5' } : {}}>
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 text-[11px] font-black tracking-tight overflow-hidden ${isSel ? 'bg-white/60' : 'bg-indigo-50'}`}>
                          {group.icon && !isDef ? <img src={group.icon} alt={group.name} className="w-full h-full object-cover rounded-xl" />
                            : isDef ? <span className={isSel ? 'text-indigo-700' : 'text-indigo-600'}>{getGroupAbbr(group)}</span>
                            : isPrivate ? <Lock className={`w-4 h-4 ${isSel ? 'text-white' : 'text-indigo-500'}`} />
                      ...
                      <button onClick={e => { e.stopPropagation(); setSidebarMenuId(sidebarMenuId === chatId ? null : chatId) }}
                        className={`absolute top-2 right-2 p-1 rounded-md transition-all ${isSel ? 'text-indigo-200 hover:text-white' : 'text-gray-600 hover:text-gray-400 hover:bg-gray-50'} ${sidebarMenuId === chatId ? 'opacity-100' : 'opacity-0 group-hover/item:opacity-100'}`}>
                      ...
                      {sidebarMenuId === chatId && (
                        <div className="absolute top-8 right-2 rounded-xl shadow-xl py-1.5 w-52 z-20" style={{ background: 'white', border: '1px solid #e5e7eb', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }} onClick={e => e.stopPropagation()}>
                          {!isDef && <button onClick={() => handlePinGroup(group.id)} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">{isPinned ? <PinOff className="w-4 h-4 text-gray-400" /> : <Pin className="w-4 h-4 text-gray-400" />}<span>{isPinned ? 'Unpin' : 'Pin to Top'}</span></button>}
                          <button onClick={() => toggleMute(chatId)} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">{muted ? <BellRing className="w-4 h-4 text-gray-500" /> : <BellOff className="w-4 h-4 text-gray-500" />}<span>{muted ? 'Unmute' : 'Mute'}</span></button>
                          {!isDef && <><div className="my-1 mx-3" style={{ borderTop: '1px solid #f3f4f6' }} /><button onClick={() => copyInviteLink(group)} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">{copiedInviteLink ? <Check className="w-4 h-4 text-green-500" /> : <Link2 className="w-4 h-4 text-gray-400" />}<span>{copiedInviteLink ? 'Copied!' : 'Copy Invite Link'}</span></button><button onClick={() => { setSidebarMenuId(null); setShareGroupViaDM(group) }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors"><Share2 className="w-4 h-4 text-gray-400" /><span>Share via DM</span></button></>}
                        </div>
                      )}
                    </div>
                  )
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {filteredDMs.map(conv => {
                  const isSel = selectedDM?.user.id === conv.user.id; const chatId = `dm_${conv.user.id}`; const muted = isMuted(chatId)
                  return (
                    <div key={conv.user.id} className="relative group/item" data-sidebar-menu>
                      <button onClick={() => handleSelectDMMobile(conv)} className={`w-full text-left p-3 rounded-xl transition-all flex items-center gap-3 ${isSel ? 'shadow-lg shadow-indigo-600/10' : 'hover:bg-gray-50'}`} style={isSel ? { background: 'rgba(79,70,229,0.08)', border: '1px solid rgba(79,70,229,0.15)' } : {}}>
                      ...
                      <button onClick={e => { e.stopPropagation(); setSidebarMenuId(sidebarMenuId === chatId ? null : chatId) }} className={`absolute top-2 right-2 p-1 rounded-md transition-all ${isSel ? 'text-indigo-400 hover:text-indigo-600 hover:bg-indigo-50' : 'text-gray-600 hover:text-gray-400 hover:bg-gray-50'} ${sidebarMenuId === chatId ? 'opacity-100' : 'opacity-0 group-hover/item:opacity-100'}`}><MoreVertical className="w-3.5 h-3.5" /></button>
                      {sidebarMenuId === chatId && (
      ...
      {/* MAIN CHAT AREA */}
      <div className="flex-1 flex flex-col min-w-0" style={{ display: 'flex', position: isMobile ? 'absolute' : 'relative', top: 0, left: 0, right: 0, bottom: isMobile ? '60px' : 0, zIndex: isMobile ? 10 : 'auto', transform: isMobile && mobileView !== 'chat' ? 'translateX(100%)' : 'translateX(0)', transition: isMobile ? 'transform 0.3s cubic-bezier(0.4,0,0.2,1)' : undefined, visibility: isMobile && mobileView !== 'chat' ? 'hidden' : 'visible' }}>
        {/* HEADER */}
        <div className="h-[64px] flex items-center justify-between flex-shrink-0" style={{ background: 'white', borderBottom: '1px solid #e5e7eb', padding: isMobile ? '0 16px' : '0 24px' }}>
          <div className="flex items-center gap-3">
            {isMobile && <button onClick={() => setMobileView('list')} className="p-1.5 -ml-1 rounded-lg text-gray-500 hover:bg-gray-50 transition-all mr-1"><ChevronLeft className="w-5 h-5" /></button>}
            {isGroupMode && selectedChat && (<>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-[11px] font-black tracking-tight text-white flex-shrink-0 overflow-hidden" style={{ background: '#4f46e5' }}>
                {selectedChat.icon && !selectedChat.isDefault ? <img src={selectedChat.icon} alt={selectedChat.name} className="w-full h-full object-cover" />
                  : selectedChat.isDefault ? <span>{getGroupAbbr(selectedChat)}</span>
            ...
            {isDMMode && selectedDM && (<>
              ...
              <div>
                <div className="flex items-center gap-2"><h1 className="text-[15px] font-bold text-gray-900">{selectedDM.user.firstName} {selectedDM.user.lastName}</h1>{currentMuted && <VolumeX className="w-3.5 h-3.5 text-gray-600" />}</div>
                <p className="text-xs text-gray-500">{onlineUsers.has(selectedDM.user.id) ? <span className="text-green-600 font-medium">Online</span> : selectedDM.user.major ? `${selectedDM.user.major}${selectedDM.user.year ? ` • ${selectedDM.user.year}` : ''}` : 'Offline'}</p>
              </div>
            </>)}
          </div>
              ...
              <div ref={headerMenuRef} className="relative">
                ...
                {showHeaderMenu && (
                  <div className="absolute top-10 right-0 rounded-xl shadow-xl py-1.5 w-52 z-20" style={{ background: 'white', border: '1px solid #e5e7eb', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
                    {isGroupMode && selectedChat && !selectedChat.isDefault && <button onClick={() => handlePinGroup(selectedChat.id)} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">{pinnedGroupId === selectedChat.id ? <PinOff className="w-4 h-4 text-gray-400" /> : <Pin className="w-4 h-4 text-gray-400" />}<span>{pinnedGroupId === selectedChat.id ? 'Unpin' : 'Pin to Top'}</span></button>}
                    <button onClick={() => toggleMute(currentChatId)} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">{currentMuted ? <><BellRing className="w-4 h-4 text-gray-500" /><span>Unmute</span></> : <><BellOff className="w-4 h-4 text-gray-500" /><span>Mute</span></>}</button>
                    {isGroupMode && selectedChat && !selectedChat.isDefault && (<>
                      <div className="my-1 mx-3" style={{ borderTop: '1px solid #f3f4f6' }} />
                      <button onClick={() => { copyInviteLink(selectedChat); setShowHeaderMenu(false) }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors"><Link2 className="w-4 h-4 text-gray-400" /><span>Copy Invite Link</span></button>
                      <button onClick={() => { setShareGroupViaDM(selectedChat); setShowHeaderMenu(false) }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors"><Share2 className="w-4 h-4 text-gray-400" /><span>Share via DM</span></button>
                      {currentUserRole === 'admin' && <button onClick={() => { handleLoadJoinRequests(selectedChat.id); setShowHeaderMenu(false) }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors"><Users className="w-4 h-4 text-gray-400" /><span>Join Requests</span></button>}
                      <div className="my-1 mx-3" style={{ borderTop: '1px solid #f3f4f6' }} />
                      {currentUserRole !== 'admin' && <button onClick={() => { setShowLeaveGroupModal(true); setShowHeaderMenu(false) }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-orange-600 hover:bg-orange-50 transition-colors"><LogOut className="w-4 h-4 text-orange-400" /><span>Leave Group</span></button>}
                      {currentUserRole === 'admin' && <button onClick={() => { setShowDeleteGroupModal(true); setShowHeaderMenu(false) }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition-colors"><Trash2 className="w-4 h-4 text-red-400" /><span>Delete Group</span></button>}
                    </>)}
                    {isGroupMode && selectedChat && selectedChat.isDefault && (<><div className="my-1 mx-3" style={{ borderTop: '1px solid #f3f4f6' }} /><button onClick={() => { setShowLeaveGroupModal(true); setShowHeaderMenu(false) }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-orange-600 hover:bg-orange-50 transition-colors"><LogOut className="w-4 h-4 text-orange-400" /><span>Leave Group</span></button></>)}
                  </div>
                )}
              </div>
            )}
        ...
        <div className="flex-1 flex overflow-hidden">
            ...
            {(isGroupMode || isDMMode) && (
              <div className="flex-shrink-0" style={{ background: 'white', borderTop: '1px solid #e5e7eb' }}>
                {Object.keys(typingUsers).length > 0 && (
                  <div className="px-6 pt-2 pb-0"><div className="flex items-center gap-2 text-xs text-gray-500"><span className="flex gap-0.5"><span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} /><span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} /><span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} /></span>
                    <span className="font-medium text-indigo-600">{(() => { const names = Object.values(typingUsers).map(t => t.name.split(' ')[0]); if (names.length === 1) return `${names[0]} is typing...`; if (names.length === 2) return `${names[0]} and ${names[1]} are typing...`; return `${names[0]} and ${names.length - 1} others are typing...` })()}</span>
                  </div></div>
                )}
          ...
          {showGroupInfo && isGroupMode && selectedChat && (
            <div className="flex flex-col flex-shrink-0 overflow-hidden" style={{ background: 'white', borderLeft: isMobile ? 'none' : '1px solid #e5e7eb', width: isMobile ? '100%' : '300px', position: isMobile ? 'absolute' : 'relative', top: 0, left: 0, right: 0, bottom: isMobile ? '60px' : 0, zIndex: isMobile ? 30 : 'auto', overflowY: isMobile ? 'auto' : undefined }}>
              <div className="h-[64px] px-4 flex items-center justify-between flex-shrink-0" style={{ borderBottom: '1px solid #e5e7eb' }}>
                {isMobile ? <button onClick={() => setShowGroupInfo(false)} className="flex items-center gap-1 text-indigo-600 font-semibold text-[15px] -ml-1 p-1.5 rounded-lg hover:bg-indigo-50 transition-all"><ChevronLeft className="w-5 h-5" /><span>Back</span></button>
              ...
              <div className="px-5 py-6 flex flex-col items-center" style={{ borderBottom: '1px solid #e5e7eb' }}>
                <div className="relative mb-3">
                  {selectedChat.icon && !selectedChat.isDefault ? <img src={selectedChat.icon} alt={selectedChat.name} className="w-16 h-16 rounded-2xl object-cover" style={{ border: '2px solid rgba(99,102,241,0.15)' }} />
                    : <div className="w-16 h-16 rounded-2xl flex items-center justify-center font-black tracking-tight" style={{ background: '#4f46e5', fontSize: selectedChat.isDefault ? '13px' : '18px' }}><span className="text-white">{selectedChat.isDefault ? getGroupAbbr(selectedChat) : getGroupInitials(selectedChat.name)}</span></div>}
                  {!selectedChat.isDefault && currentUserRole === 'admin' && (
                    <button onClick={() => groupAvatarInputRef.current?.click()} disabled={isUploadingGroupAvatar} className="absolute -bottom-1.5 -right-1.5 w-7 h-7 rounded-full flex items-center justify-center shadow-md border-2 border-white transition-all hover:scale-110 disabled:opacity-60" style={{ background: '#4f46e5' }}>
                      {isUploadingGroupAvatar ? <Loader2 className="w-3.5 h-3.5 text-white animate-spin" /> : <Camera className="w-3.5 h-3.5 text-white" />}
                    </button>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {selectedChat.visibility === 'private' ? <span className="text-[10px] font-bold text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded-md uppercase flex items-center gap-1"><Lock className="w-3 h-3" />Private</span>
                ...
                {!selectedChat.isDefault && selectedChat.inviteCode && (
                  <div className="mt-4 w-full flex flex-col items-center gap-3">
                    <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl w-full" style={{ background: '#f9fafb', border: '1px solid #e5e7eb' }}>
                      <div className="flex-1 min-w-0"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Invite Code</p><p className="text-[15px] font-mono font-bold text-gray-900 tracking-widest">{selectedChat.inviteCode}</p></div>
                      <button onClick={() => { navigator.clipboard.writeText(selectedChat.inviteCode || ''); setCopiedInviteCode(true); setTimeout(() => setCopiedInviteCode(false), 2000) }} className="p-2 rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all">{copiedInviteCode ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}</button>
                    </div>
                    <div className="flex items-center gap-2">
                      <button onClick={() => copyInviteLink(selectedChat)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 transition-all">{copiedInviteLink ? <><Check className="w-3.5 h-3.5" />Copied!</> : <><Link2 className="w-3.5 h-3.5" />Copy Invite Link</>}</button>
                      <button onClick={() => setShareGroupViaDM(selectedChat)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 transition-all"><Share2 className="w-3.5 h-3.5" />Share via DM</button>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="px-5 py-2 flex gap-2">
                <select value={memberYearFilter} onChange={e => setMemberYearFilter(e.target.value)} className="flex-1 px-2 py-1.5 rounded-lg text-xs text-gray-400 focus:outline-none" style={{ background: '#f9fafb', border: '1px solid #e5e7eb', appearance: 'none' }}><option value="">All Years</option>{getAvailableYears().map(y => <option key={y} value={y}>{y}</option>)}</select>
                <select value={memberSemesterFilter} onChange={e => setMemberSemesterFilter(e.target.value)} className="flex-1 px-2 py-1.5 rounded-lg text-xs text-gray-400 focus:outline-none" style={{ background: '#f9fafb', border: '1px solid #e5e7eb', appearance: 'none' }}><option value="">All Semesters</option>{getAvailableSemesters().map(s => <option key={s} value={s}>{s}</option>)}</select>
              </div>
                ...
                <div className="space-y-1">
                  {filteredMembers.map(member => (
                    <div key={member.id} className="flex items-center gap-3 py-2.5 rounded-lg hover:bg-gray-50 px-1 transition-all">
                      ...
                      {member.user.id !== user?.id && <button onClick={() => handleStartDM({ id: member.user.id, firstName: member.user.firstName, lastName: member.user.lastName, profileImage: member.user.profileImage, major: member.user.major, year: member.user.year })} className="p-1.5 text-gray-500 hover:text-indigo-500 hover:bg-indigo-600/10 rounded-lg transition-all"><MessageCircle className="w-4 h-4" /></button>}
                    </div>
                  ))}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowCreateGroup(false)}>
          <div className="rounded-2xl shadow-2xl max-w-md w-full p-6" style={{ background: 'white', border: '1px solid #e5e7eb' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="flex justify-center mb-5">
              <div className="relative">
                <div className="w-20 h-20 rounded-2xl flex items-center justify-center overflow-hidden cursor-pointer relative group/avatar" style={{ background: groupForm.icon ? 'transparent' : 'linear-gradient(135deg,#6366f1,#8b5cf6)', border: '2px dashed #c7d2fe' }} onClick={() => groupImageInputRef.current?.click()}>
                  {groupForm.icon ? <img src={groupForm.icon} alt="" className="w-full h-full object-cover rounded-2xl" /> : <div className="flex flex-col items-center gap-1"><Camera className="w-6 h-6 text-white/80" /><span className="text-[9px] text-white/70 font-medium">Add Photo</span></div>}
                  <div className="absolute inset-0 bg-black/30 rounded-2xl opacity-0 group-hover/avatar:opacity-100 transition-opacity flex items-center justify-center"><Camera className="w-5 h-5 text-white" /></div>
                </div>
                {groupForm.icon && <button onClick={e => { e.stopPropagation(); setGroupForm(p => ({ ...p, icon: '' })) }} className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center shadow-md hover:bg-red-600"><X className="w-3 h-3" /></button>}
                <input ref={groupImageInputRef} type="file" accept="image/*" className="hidden" onChange={e => {
                  const file = e.target.files?.[0]; if (!file) return; e.target.value = ''
                  const reader = new FileReader(); reader.onload = () => { const img = new Image(); img.onload = () => { const canvas = document.createElement('canvas'); const size = Math.min(img.width, img.height); canvas.width = 300; canvas.height = 300; const ctx = canvas.getContext('2d')!; ctx.drawImage(img, (img.width - size) / 2, (img.height - size) / 2, size, size, 0, 0, 300, 300); setGroupForm(p => ({ ...p, icon: canvas.toDataURL('image/jpeg', 0.85) })) }; img.src = reader.result as string }; reader.readAsDataURL(file)
                }} />
              </div>
            </div>
      ...
      {showJoinModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => { setShowJoinModal(false); setJoinCode(''); setJoinError('') }}>
          <div className="rounded-2xl shadow-2xl max-w-sm w-full p-6" style={{ background: 'white', border: '1px solid #e5e7eb' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-4"><input type="text" value={joinCode} onChange={e => { setJoinCode(e.target.value); setJoinError('') }} onKeyDown={e => { if (e.key === 'Enter') handleJoinGroup() }} placeholder="Enter invite code..." autoFocus className={`w-full px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600/30 text-gray-900 placeholder-gray-400 ${joinError ? 'ring-1 ring-red-500' : ''}`} style={{ background: 'white', border: '1px solid #e5e7eb' }} />{joinError && <p className="text-xs text-red-400 flex items-center gap-1 mt-1.5"><AlertCircle className="w-3.5 h-3.5" />{joinError}</p>}</div>
            <div className="flex gap-3"><button onClick={() => { setShowJoinModal(false); setJoinCode(''); setJoinError('') }} className="flex-1 px-4 py-2.5 rounded-xl text-gray-700 font-semibold text-sm hover:bg-gray-50" style={{ border: '1px solid #d1d5db' }}>Cancel</button><button onClick={handleJoinGroup} disabled={isJoining || !joinCode.trim()} className="flex-1 px-4 py-2.5 text-white rounded-xl font-semibold text-sm disabled:opacity-50 flex items-center justify-center gap-2" style={{ background: '#4f46e5' }}>{isJoining ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Join Group'}</button></div>
          </div>
        </div>
      )}
      ...
      {forwardingMessage && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setForwardingMessage(null)}>
          <div className="rounded-2xl shadow-2xl max-w-md w-full p-6 max-h-[500px] flex flex-col" style={{ background: 'white', border: '1px solid #e5e7eb' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto space-y-1">
              {groups.map(g => <button key={g.id} onClick={() => forwardToChat(g)} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-all"><div className="w-9 h-9 rounded-lg bg-indigo-50 flex items-center justify-center"><Users className="w-4 h-4 text-indigo-600" /></div><span className="text-sm font-medium text-gray-900">{g.name}</span></button>)}
              {dmConversations.map(c => <button key={c.user.id} onClick={() => forwardToDM(c)} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-all"><UserAvatar src={c.user.profileImage} firstName={c.user.firstName} lastName={c.user.lastName} size={36} /><span className="text-sm font-medium text-gray-900">{c.user.firstName} {c.user.lastName}</span></button>)}
            </div>
          </div>
        </div>
      )}
      ...
      {showDeleteGroupModal && selectedChat && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowDeleteGroupModal(false)}>
          <div className="rounded-2xl shadow-2xl max-w-sm w-full p-6" style={{ background: 'white', border: '1px solid #e5e7eb' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3"><button onClick={() => setShowDeleteGroupModal(false)} className="flex-1 px-4 py-2.5 text-gray-700 rounded-xl font-semibold text-sm hover:bg-gray-50" style={{ border: '1px solid #d1d5db' }}>Cancel</button><button onClick={handleDeleteGroup} disabled={isDeletingGroup} className="flex-1 px-4 py-2.5 text-white rounded-xl font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-50" style={{ background: '#ef4444' }}>{isDeletingGroup ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Delete Group'}</button></div>
          </div>
        </div>
      )}
      ...
      {showLeaveGroupModal && selectedChat && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowLeaveGroupModal(false)}>
          <div className="rounded-2xl shadow-2xl max-w-sm w-full p-6" style={{ background: 'white', border: '1px solid #e5e7eb' }} onClick={e => e.stopPropagation()}>
            <div className="w-12 h-12 rounded-full bg-orange-100 flex items-center justify-center mx-auto mb-4"><LogOut className="w-6 h-6 text-orange-600" /></div>
            ...
            <div className="flex gap-3"><button onClick={() => setShowLeaveGroupModal(false)} className="flex-1 px-4 py-2.5 text-gray-700 rounded-xl font-semibold text-sm hover:bg-gray-50" style={{ border: '1px solid #d1d5db' }}>Cancel</button><button onClick={handleLeaveGroup} className="flex-1 px-4 py-2.5 text-white rounded-xl font-semibold text-sm" style={{ background: '#f97316' }}>Leave Group</button></div>
          </div>
        </div>
      )}
      ...
      {showJoinRequests && selectedChat && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowJoinRequests(false)}>
          <div className="rounded-2xl shadow-2xl max-w-md w-full p-6 flex flex-col max-h-[500px]" style={{ background: 'white', border: '1px solid #e5e7eb' }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-bold text-gray-900">Join Requests</h2><button onClick={() => setShowJoinRequests(false)} className="p-1 hover:bg-gray-50 rounded-lg text-gray-500"><X className="w-5 h-5" /></button></div>
            <div className="flex-1 overflow-y-auto space-y-2">
              {joinRequests.length === 0 ? <div className="text-center py-10"><div className="w-12 h-12 rounded-full bg-gray-50 flex items-center justify-center mx-auto mb-3"><Users className="w-6 h-6 text-gray-300" /></div><p className="text-sm text-gray-400 font-medium">No pending requests</p><p className="text-xs text-gray-300 mt-1">All caught up!</p></div>
                : joinRequests.map(req => (
                  <div key={req.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: '#f9fafb', border: '1px solid #e5e7eb' }}>
                    ...
                    <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-gray-900">{req.user.firstName} {req.user.lastName}</p><p className="text-xs text-gray-500">{req.user.major}{req.user.year ? ` • ${req.user.year}` : ''}</p></div>
                    <div className="flex gap-2">
                      <button onClick={() => handleApproveReject(req.user.id, 'reject')} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-red-600 hover:bg-red-50 transition-all" style={{ border: '1px solid #fca5a5' }}>Reject</button>
                      <button onClick={() => handleApproveReject(req.user.id, 'approve')} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-white transition-all" style={{ background: '#4f46e5' }}>Approve</button>
                    </div>
                  </div>
                ))}
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setPendingApproval(null)}>
          <div className="rounded-2xl shadow-2xl max-w-sm w-full p-6" style={{ background: 'white', border: '1px solid #e5e7eb' }} onClick={e => e.stopPropagation()}>
            <div className="flex flex-col items-center text-center">
              <div className="w-14 h-14 rounded-full bg-amber-50 flex items-center justify-center mb-4"><span className="text-2xl">⏳</span></div>
              <h2 className="text-lg font-bold text-gray-900 mb-2">Request Sent!</h2>
              <p className="text-sm text-gray-500 mb-1">Your request to join</p>
              <p className="text-base font-bold text-indigo-600 mb-3">"{pendingApproval.groupName}"</p>
              <p className="text-sm text-gray-500 mb-6">is pending admin approval. You'll receive a notification once you're approved.</p>
              <button onClick={() => setPendingApproval(null)} className="w-full px-4 py-2.5 text-white rounded-xl font-semibold text-sm" style={{ background: '#4f46e5' }}>Got it</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isMobile && (
        <nav className="fixed bottom-0 left-0 right-0 z-40 flex items-center border-t" style={{ background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderColor: '#e5e7eb', paddingBottom: 'env(safe-area-inset-bottom,0px)', height: '60px' }}>
          ...
          <button onClick={() => router.push('/home/campus-talks')} className="flex-1 flex flex-col items-center justify-center gap-0.5 py-2 transition-all">
            <div className="p-1.5 rounded-xl"><Megaphone className="w-[22px] h-[22px] text-gray-400" /></div>
          ...
          <button onClick={() => setShowLogoutModal(true)} className="flex-1 flex flex-col items-center justify-center gap-0.5 py-2 transition-all">
            <div className="p-1.5 rounded-xl">
              {user?.profileImage ? <img src={user.profileImage} className="w-[22px] h-[22px] rounded-full object-cover" /> : <div className="w-[22px] h-[22px] rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-[10px]">{user?.firstName?.[0]}{user?.lastName?.[0]}</div>}
            </div>
...
function EmptyChat({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="flex flex-col items-center justify-center h-full" style={{ background: 'linear-gradient(160deg,#f0f0ff 0%,#e8eeff 40%,#f5f0ff 70%,#eef0ff 100%)' }}>
      <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4" style={{ background: 'rgba(255,255,255,0.6)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.8)', boxShadow: '0 4px 16px rgba(99,102,241,0.08)' }}><Send className="w-7 h-7 text-indigo-400" /></div>
      <p className="text-base font-bold text-gray-400">{title}</p>
